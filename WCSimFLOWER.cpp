...
const float WCSimFLOWER::fEffCoverages[9] = {
    0.4,    0.4,    0.4,    0.4, 0.4068,
    0.4244, 0.4968, 0.5956, 0.67}; // from MC: coverage at theta = 5, 15, ..., 85 degree

WCSimFLOWER::WCSimFLOWER(const char *detectorname, WCSimRootGeom *geom, bool get_npmts_from_wcsimrootgeom,
                         bool overwrite_nearest, int verbose)
    : fLightGroupSpeed(
          21.58333), // speed of light in water, value from
                     // https://github.com/hyperk/hk-BONSAI/blob/d9b227dad26fb63f2bfe80f60f7f58b5a703250a/bonsai/hits.h#L5
      fLambdaEff(100 * 100), // scattering length in cm (based on Design Report II.2.E.1)
      fDetectorName(detectorname), fDetector(DetectorEnumFromString(detectorname)), fVerbose(0), fGeom(geom),
      fGetNPMTsFromWCSimRootGeom(get_npmts_from_wcsimrootgeom), fLongDuration(100), fShortDuration(50) {
    cout << endl << "******************" << endl << "Inialising FLOWER" << endl << "******************" << endl;
    switch (fDetector) {
    case kSuperK:
        fDarkRate = 4.2;
        fNPMTs = 11146;
    ...
    cout << "\tUsing default short duration to look for max number of hits " << fShortDuration << " ns" << endl;

    cout << "\tUsing default long duration to look for late hits " << fLongDuration << " ns" << endl;
...
void WCSimFLOWER::SetTopBottomDistance(float hi, float lo) {
    if (abs(lo) <= abs(hi)) {
        ...
        cerr << "Second argument should have the smallest absolute value. This has been handled, but swap your "
                "arguments to supress this warning"
             << endl;
    }
    cout << "Set abs(z)-cut to get barrel PMTs near caps that have fewer neighbours " << fTopBottomDistanceLo
         << " to " << fTopBottomDistanceHi << endl;
}
...
void WCSimFLOWER::CorrectHitTimes() {
    ...
    for (unsigned int i = 0; i < fNDigits; i++) {
        ...
        y = pmt.GetPosition(1);
        z = pmt.GetPosition(2);
        fDistance[i] = sqrt(pow((x - fVertex[0]), 2) + pow((y - fVertex[1]), 2) + pow((z - fVertex[2]), 2));
...
void WCSimFLOWER::FindMaxTimeInterval() {
    ...
    if (fVerbose)
        std::cout << "Maximum of " << fNMaxShort << " hits in " << fShortDuration << " ns window starting at "
                  << fStartTime << " ns" << std::endl
                  << "In extended " << fLongDuration << " ns window there are " << fNMaxLong << " hits"
                  << std::endl;
}

void WCSimFLOWER::GetNearestNeighbours(bool overwrite_root_file) {
    TString fname = TString::Format("%s/flower_nn_%s_%.5f.root", GetFLOWERDataDir().Data(), fDetectorName.c_str(),
                                    fNeighbourDistance);
    TFile f(fname);
    TTree *t;
    int tubeID;
    if (!overwrite_root_file && !f.IsZombie()) {
        ...
        if (n_entries != fNPMTs) {
            std::cerr << "Mismatch between number of PMTs FLOWER is assuming: " << fNPMTs
                      << " and number of PMTs in the previously calculated nearest neighbours file: " << n_entries
                      << " Exiting..." << endl;
            exit(-1);
        }
        ...
        for (unsigned int ipmt = 0; ipmt < fNPMTs; ipmt++) {
            ...
            for (unsigned int jpmt = 0; jpmt < fNPMTs; jpmt++) {
                if (jpmt == ipmt)
                    continue; // don't count the current PMT itself
                otherPMT = fGeom->GetPMT(jpmt);
                tubeID_j = otherPMT.GetTubeNo();
                if (sqrt(pow(x - otherPMT.GetPosition(0), 2) + pow(y - otherPMT.GetPosition(1), 2) +
...
void WCSimFLOWER::GetNEff() {
    ...
    for (unsigned int i = 0; i < fNMaxShort; i++) {
        ...
        // this depends on angle of incidence, see Fig. 4.5 (left) of
        ...
        float dotProduct = pmt.GetOrientation(0) * (fVertex[0] - x) + pmt.GetOrientation(1) * (fVertex[1] - y) +
                           pmt.GetOrientation(2) * (fVertex[2] - z);
        float theta = acos(dotProduct / fDistanceShort[i]) * 180 / 3.14159265;
        if (!((theta >= 0.) and (theta <= 89.99)))
            theta = 0; // we have apparently mis-reconstructed the vertex, so let's set ...
...
TString WCSimFLOWER::GetFLOWERDataDir() {
    ...
    if (!dir.Length() || !gSystem->OpenDirectory(dir.Data())) {
        cout << "$FLOWERDATADIR not set, or points to directory that doesn't exist. Using $FLOWERDIR/data/"
             << endl;
...
bool WCSimFLOWER::CheckNearestNeighbours() {
    ...
    for (auto const &count : counts) {
        ...
        if (count.first <= 2) {
            cerr << "Some PMTs have " << count.first << " neighbours. Increase neighbour distance & try again"
                 << endl;
            success = false;
        } else if (count.first >= 12) {
            cerr << "Some PMTs have " << count.first << " neighbours. Decrease neighbour distance & try again"
                 << endl;
            success = false;
        }
    }
